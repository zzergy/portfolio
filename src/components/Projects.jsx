    ...
    <motion.div variants={fadeIn('up', 'spring', index * 0.5, 0.75)}>
      ...
      >
        <div className="relative w-full h-[230px] flex justify-center">
          <img onClick={() => projectLink && window.open(projectLink, '_blank')} src={image} alt={name} className="object-contain rounded-2xl cursor-pointer" />
        ...
        <div className="mt-4 flex flex-col text-[14px]">
          ...
          <div className=" mt-1 flex flex-wrap gap-2">
            {tags.map(tag => (
              <div key={tag.name} className={`text-[14px] ${tag.color}`}>{tag.name}</div>
            ))}
          </div>
        </div>
      </Tilt>
    ...
    <>
      ...
      <div className="w-full flex">
        <motion.p variants={fadeIn('', '', 0.1, 1)}
          ...
          real-world examples of my work. Each project is briefly described with links to code repositories and live demos.
        </motion.p>
      </div>
